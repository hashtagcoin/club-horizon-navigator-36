...
export default function ClubPilot() {
  ...
  const toggleList = () => {
    ...
    api.start({ 
      x: !isListCollapsed ? -window.innerWidth * 0.25 + 20 : 0,
      immediate: false,
  ...
  const bind = useDrag(({ movement: [mx], direction: [dx], cancel, active }) => {
    if (active && Math.abs(mx) > window.innerWidth * 0.15) {
      ...
      api.start({ 
        x: shouldCollapse ? -window.innerWidth * 0.25 + 20 : 0, 
        immediate: false,
        config: { tension: 200, friction: 25 }
      });
    } else {
      api.start({ 
        x: active ? mx : isListCollapsed ? -window.innerWidth * 0.25 + 20 : 0, 
        immediate: active,
        config: { tension: 200, friction: 25 }
      });
    }
    ...
    axis: 'x',
    bounds: { left: -window.innerWidth * 0.25 + 20, right: 0 },
    rubberband: true,
    from: () => [x.get(), 0]
  });
